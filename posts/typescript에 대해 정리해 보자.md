...
`unknown`은 `any`와 달리 `any` type이 아닌 다른 모든 타입에 할당될 수 없는 차이가 있다. 어떤 타입이든 할 당될 수 있는 `any` 보다는 `unknown`을 선호
...
※ 임시 변수도 선언시 타입이 형성된다. ts language support로 임시 변수에 마우스 오버시 어떤 타입으로 추론되었는지 알 수 있다
...
문제점
- 빈 배열이면 `min`, `max`가 undefined로 처리된다. `[undefined, undefined]`
