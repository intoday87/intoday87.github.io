...
type k = keyof(A | B) // never -> 구조적 타이핑 관점에서 추가적인 속성이 있을수 있는 타입에서 A | B = U(전체집합)이 된다. 그러므로 A, B의 합집합은
...
단순히 합집합이라고 하면 모든 키가 합산되어 keyof에 나열되어야 할 것 같지만 never인 이유는 구조적 타이핑 관점에서 생각해봐야 한다. *모든 타입은 선언된 속성 말고도 추가적인 속성을 포함할 수 있다* 의 관점으로 생각해보면 `A | U`는 추가적은 속성을 포함한 전체 집합이 되기 때문에 각각의 타입에 속한 타입만 있다고 